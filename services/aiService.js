...
function bufferToGenerativePart(buffer, mimeType) {
    if (!(buffer instanceof Buffer)) {
        throw new Error("Invalid image data provided. Expected a buffer.");
    }
    return {
        inlineData: {
...
const analyzeReport = async (textDescription, imageBuffer, imageMimeType) => {
    try {
        ...
        const prompt = `
            ...
            Only if Step 1 fails, analyze the IMAGE for visual evidence of civic issues.
            ...
            Apply the prime directive with maximum sensitivity. Respond with ONLY the final JSON object.
        ...
        // FIX: Cleaned up the fallback object to match the schema perfectly
        return {
            isRelevant: true,
